...
class MediumGame: UIViewController, subviewDelegate {
    ...
    @IBOutlet weak var MediumGozillaDrag: MediumDrag!
    
    @IBOutlet weak var MovingRoadImages2: UIImageView!
    ...
    var misslesArray = [0.7,0.8,1.0,1.7,1.8,2.2,2.6,3.0,3.7,3.8,7.2,7.6,5.0,5.7,5.8,6.2,6.7,6.8,7.2,7.6,8.0,8.7,8.8,9.2,9.6,10.0,10.7,10.8,11.2,11.6,12.0,12.7,12.8,13.2,13.6,17.0,17.7,17.8,15.2,15.6,16.0,16.7,16.8,17.2,17.6,18.0,18.7,18.8,19.2]
    ...
    func changeSomething() {
        ...
        collisionBehavior.addBoundary(withIdentifier: "Collision Gooooozilla" as NSCopying, for: UIBezierPath(rect: MediumGozillaDrag.frame) )
        for count in Empty{
            if (MediumGozillaDrag.frame.intersects(count.frame)){
                if(scoreCounter > 1){
                ...
                ExplosionOnHit.frame = CGRect(x:MediumGozillaDrag.center.x, y: MediumGozillaDrag.center.y, width: 100 , height: 120)
    ...
    override func viewDidLoad() {
        ...
        MediumGozillaDrag.myDelegate = self
        onHitExplosion()
        ...
        MovingRoadImages2.image = UIImage.animatedImage(with: imageArray, duration: 2.0)
        MovingRoadImages2.frame = UIScreen.main.bounds
        ...
        MediumGozillaDrag.image = UIImage.animatedImage(with: draggedGodzilla, duration: 0.5)
        //  DraggingGodzilla.frame = UIScreen.main.bounds
        MediumGozillaDrag.frame = CGRect(x:130, y: 400, width: 50 , height: 80)
